    ...
    $("#filter-account, #filter-category, #filter-type").change(async () => {
        try {
            ...
            const filterAccount =
                $("#filter-account").val() === ""
                    ? ""
                    : parseInt($("#filter-account").val());
            const filterCategory =
                $("#filter-category").val() === ""
                    ? ""
                    : parseInt($("#filter-category").val());
            const filterType = $("#filter-type").val();

            //filter transactions
            const accountFilteredTransactions = accountFilters(
                filterAccount,
                flatTransactions
            );
            const categoryFilteredTransactions = categoryFilters(
                filterCategory,
                accountFilteredTransactions
            );
            const typeFilteredTransactions = typeFilters(
                filterType,
